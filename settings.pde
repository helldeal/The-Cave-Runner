...
void setting(){
  ...
  //script
  if(mouseButton==LEFT&&mouseX<width*3/5&&mouseX>width*2/5&&mouseY<height*7.3/8&&mouseY>height*6.7/8){page="prestart";transstart=0;} 
...
void settext(){
  ...
  if(settings=="video"){
    ...
    if(reso==3)text("<1080>",width/2+600*width/2560,height*3/8);
    if(reso==2)text("<720>",width/2+600*width/2560,height*3/8);
  if(reso<1)reso=3;
  if(reso>3)reso=1;
  //display
